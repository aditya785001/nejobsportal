...
export default function ScholarshipsPage() {
  ...
  const updateFilter = (key: string, value: string) => {
    ...
    params.delete("page");
    router.push(`/scholarships?${params}`);
    setPage(1);
  };
    ...
    <div className="container-main py-8">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl md:text-3xl font-bold">
                  {stateDisplayNames[state] || state} {t("scholarships.title", language)}
                </h1>
                <p className="text-green-100 text-sm">
                  {t("scholarships.subtitle", language)}
                </p>
              </div>
            </div>
            <Link
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">{t("scholarships.no-results", language)}</h3>
          <p className="text-gray-600">{t("scholarships.no-results.hint", language)}</p>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {scholarships.map((s) => {
              const title = language === "as" ? s.titleAs : s.titleEn;
              return (
                <Link
                  key={s.id}
                ...
                >
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                    <div className="flex gap-1">
                      {s.hashtagList.slice(0, 2).map((tag) => (
